...
fig, ax = plt.subplots(
    figsize=(14, 7), ncols=3, nrows=2
)  # Изменяем размер сетки на 2 строки и 3 столбца
...
# Добавляем график для значений потерь (loss)
ax[1][1].plot(range(len(optimizer.losses)), optimizer.losses)
