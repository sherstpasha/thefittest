...
from thefittest.optimizers._selfcgakf import SelfCGAKF
...
def run_optimization(F, content, n_vars, eps, iters_pop, K):
    ...
    optimizer = SelfCGAKF(fitness_function=content["function"](),
...
def main():
    ...
    combined_df.to_csv("combined_results_selfcgakf.csv", index=False)

    iters_pop_df = pd.DataFrame(list(iters_pop.items()), columns=["Function", "Iters_Pop_Size"])
    iters_pop_df.to_csv("iters_pop_size_selfcgakf.csv", index=False)
