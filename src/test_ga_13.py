            ...
            for selection in ["proportional", "rank", "tournament_3", "tournament_5", "tournament_7"]:
                for crossover in ["one_point",
                    ...
                    for mutation in ["weak", "average", "strong"]:
                        for n_vars in content["dimentions"]:  # Возвращаем цикл по n_vars
                            futures = [pool.apply_async(run_optimization, args=(F, content, n_vars, eps, iters_pop, selection, crossover, mutation)) for _ in range(n_runs)]
