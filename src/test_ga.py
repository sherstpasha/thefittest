...
def find_solution_with_precision(solution_list, true_solution, precision):
    ...
    for i, solution in enumerate(solution_list):
        ...
        error = np.abs(solution - true_solution[:10])
...
def run_optimization(function, eps, iters, pop_size, selection, crossover, mutation):
    ...
    left = np.array([function["bounds"][0]] * 10, dtype=np.float64)
    right = np.array([function["bounds"][1]] * 10, dtype=np.float64)
    h = np.array([eps] * 10, dtype=np.float64)
...
def main():
    ...
    while pop_size <= max_pop_size and iters <= max_iters:
        ...
        pop_size = pop_size + int(pop_size * 0.7)
