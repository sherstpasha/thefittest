...
class UniversalSetO:
    def __init__(self, functional_set, terminal_set, ephemeral_set=[]):
        self.functional_set = {'any': {}}
        for unit in functional_set:
            self.functional_set['any'][unit.name] = unit

        for unit in functional_set:
            n_args = unit.n_args
            if n_args not in self.functional_set:
                self.functional_set[n_args] = {unit.name: unit}
            else:
                self.functional_set[n_args][unit.name] = unit
        ...
        self.o_sets = {key: value for key, value
                      in self.functional_set.items() if key != 'any'}
        self.o_proba = self.define_o_proba()

    def define_o_proba(self):
        o_proba = {}
        for key, value in self.o_sets.items():
            print(key, value)
            z_i = len(value)
            

            o_proba[key] = dict(zip(list(value.keys()), np.full(z_i, 1/z_i)))
        print(o_proba)
        return o_proba

    def choice_operator(self, proba_dict):
        operators = list(proba_dict.keys())
        proba = list(proba_dict.values())
        return random.choices(list(operators), weights=proba)[0]
    ...
    def random_functional(self, n_args='any'):
        if n_args == 'any':
            n_args = random.randrange(1, len(self.functional_set))
        name = self.choice_operator(self.o_proba[n_args])
        return self.functional_set[n_args][name]