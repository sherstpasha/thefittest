...
class SelfCGAKF(GeneticAlgorithm):
    ...
    def _choice_operators(self: SelfCGAKF, proba_dict: Dict["str", float]) -> NDArray:
    ...
    def _get_new_probakf(
        ...
        for operator_i in proba_dict.keys():
            if operator_i == operator:
                proba_dict[operator_i] = kf_value
            else:
                proba_dict[operator_i] = (1 - kf_value) / (len(proba_dict) - 1)
    ...
    def _find_fittest_operator(
        ...
        argmax = np.argmax(mean_fit)
        fittest_operator = keys[argmax]
    ...
    def _get_new_population(self: SelfCGAKF) -> None:
