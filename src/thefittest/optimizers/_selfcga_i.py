...
class SelfCGA_i(GeneticAlgorithm):
    ...
    def __init__(
        ...
        self._selection_operators_max = self._choice_most_probable_operator(
            proba_dict=self._selection_proba
        )
        self._crossover_operators_max = self._choice_most_probable_operator(
            proba_dict=self._crossover_proba
        )
        self._mutation_operators_max = self._choice_most_probable_operator(
            proba_dict=self._mutation_proba
        )

    def _choice_operators(self: SelfCGA_i, proba_dict: Dict["str", float]) -> NDArray:
    ...
    def _choice_most_probable_operator(self, proba_dict: Dict["str", float]) -> np.ndarray:
        most_probable_operator = min(proba_dict, key=proba_dict.get)
        return most_probable_operator
    ...
    def _find_fittest_operator(
        self: SelfCGA_i, operators: NDArray, fitness: NDArray[np.float64]
    ) -> str:
    ...
    def _update_data(self: SelfCGA_i) -> None:
        super()._update_data()
        self._update_stats(
            s_proba=self._selection_proba,
            c_proba=self._crossover_proba,
            m_proba=self._mutation_proba,
            s_max=self._selection_operators_max,
            c_max=self._crossover_operators_max,
            m_max=self._mutation_operators_max,
        )

    def _get_fitness(self, population_ph):
        fitness = super()._get_fitness(population_ph)
        if self._i % 1 != 0:
            self._fitness_probas_i = fitness.copy()

        return fitness

    def _adapt(self: SelfCGA_i) -> None:
        if self._i % 1 != 0:
            s_fittest_oper = self._find_fittest_operator(
                self._selection_operators, self._fitness_probas_i
            )
            ...
            c_fittest_oper = self._find_fittest_operator(
                self._crossover_operators, self._fitness_probas_i
            )
            ...
            m_fittest_oper = self._find_fittest_operator(
                self._mutation_operators, self._fitness_probas_i
            )
    ...
    def _get_new_population(self: SelfCGA_i) -> None:
        if self._i % 1 != 0:
            ...
            self._population_g_i = np.array(
                [
                    self._get_new_individ_g(
                        self._selection_operators_max,
                        self._crossover_operators_max,
                        self._mutation_operators_max,
                    )
                    for i in range(self._pop_size)
                ],
                dtype=self._population_g_i.dtype,
