...
def fitness_function(
    ...
    for i, ensemble in enumerate(population):
        ...
        output2d = ensemble.meta_output(X)[:, 0]
        fitness[i] = root_mean_square_error(targets.cpu().numpy(), output2d.cpu().numpy())
        print(i, fitness[i])
    return fitness
...
def evaluate_nets(
    ...
    error = root_mean_square_error2d(targets, output2d).cpu().numpy()
    return error
...
class GeneticProgrammingNeuralNetStackingRegressor(GeneticProgrammingNeuralNetRegressor):
    def __init__(
        self,
        iters: int,
        pop_size: int,
    ...
    def _get_uniset_1(
        self: GeneticProgrammingNeuralNetStackingRegressor, X: NDArray[np.float32]
    ...
    def _define_optimizer_ensembles(
        self: GeneticProgrammingNeuralNetStackingRegressor,
        uniset: UniversalSet,
        n_outputs: int,
        X_train_ens: NDArray[np.float32],
        proba_train_ens: NDArray[np.float32],
