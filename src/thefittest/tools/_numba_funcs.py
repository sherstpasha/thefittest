...
def find_end_subtree_from_i(index: np.int64,
                            n_args_array: NDArray[np.int64]) -> np.int64:
    n_index = index + 1
...
@njit(int64[:](int64, int64[:]))
def find_id_args_from_i(index: np.int64,
                        n_args_array: NDArray[np.int64]) -> NDArray[np.int64]:
...
def binary_search_interval(value: np.float64,
                           intervals: NDArray[np.float64]) -> np.int64:
...
def check_for_value(value: np.int64,
                    index_array: NDArray[np.int64],
                    end: np.int64) -> bool:
    found = False
    for i in range(end):
