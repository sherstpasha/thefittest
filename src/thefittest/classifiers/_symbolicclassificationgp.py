...
from ..tools.metrics import root_mean_square_error, categorical_crossentropy
...
def fitness_function(trees: NDArray, y: NDArray[np.float32]) -> NDArray[np.float32]:
    ...
    for tree in trees:
        ...
        fitness.append(categorical_crossentropy(y.astype(np.float32), proba_predict.astype(np.float32)))
...
class SymbolicClassificationGP(Model):
    def __init__(
        self,
        iters: int,
        pop_size: int,
    ...
    def _get_uniset(self: SymbolicClassificationGP, X: NDArray[np.float32]) -> UniversalSet:
        uniset: UniversalSet
        terminal_set: Union[
    ...
    def _fit(
        self: SymbolicClassificationGP, X: NDArray[np.float32], y: NDArray[Union[np.float32, np.int64]]
    ) -> SymbolicClassificationGP:
    ...
    def _predict(
        self: SymbolicClassificationGP, X: NDArray[np.float32]
        ...
        tree_output = genotype_for_pred() * np.ones(len(X))
        proba = safe_sigmoid(tree_output)
