...
def main():
    ...
    reliability_df.to_csv("reliability_results_selfaga_v1.csv", index=False)
    speed_df.to_csv("speed_results_selfaga_v1.csv", index=False)
    range_df.to_csv("range_results_selfaga_v1.csv", index=False)

    iters_pop_df = pd.DataFrame(list(iters_pop.items()), columns=["Function", "Iters_Pop_Size"])
    iters_pop_df.to_csv("iters_pop_size_selfaga_v1.csv", index=False)
