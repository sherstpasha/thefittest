...
def run_optimization(F, content, n_vars, eps, iters_pop, adapt_oper, mutate_operator_proba):
    ...
    optimizer = MyAdaptGA(fitness_function=content["function"](),
...
def main():
    ...
    mutate_operator_proba_range = np.arange(0, 1.05, 0.05)
            ...
            for apart_operator in adaptation_operator_list:
                for mutate_operator_proba_i in mutate_operator_proba_range:
                    for n_vars in content["dimentions"]:
                        futures = [pool.apply_async(run_optimization, args=(F, content, n_vars, eps, iters_pop, apart_operator, mutate_operator_proba_i)) for _ in range(n_runs)]
                        ...
                        results.append([F, n_vars, apart_operator, mutate_operator_proba_i, reliability, speed, range_left, range_right])

                    progress_bar.update(1)

    progress_bar.close()

    combined_df = pd.DataFrame(results, columns=["Function", "Dimensions", "Adaptation_Operator", "Mutate_Operator_Proba", "Reliability", "Speed", "Range_Left", "Range_Right"])
    combined_df.to_csv("combined_results_my_adapt_ga.csv", index=False)

    iters_pop_df = pd.DataFrame(list(iters_pop.items()), columns=["Function", "Iters_Pop_Size"])
    iters_pop_df.to_csv("iters_pop_size_my_adapt_ga.csv", index=False)
