...
from cec2022 import problems_tuple
...
def run_optimization(function, eps, iters, pop_size, selection, crossover, mutation, dim):
    ...
    def fitness_(pop):
        results = np.zeros(len(pop), dtype=np.float32)
        for i, pip_i in enumerate(pop):
            results[i] = function["function"].evaluate(pip_i)
        return results

    left = np.array([function["bounds"][0]] * dim, dtype=np.float32)
    right = np.array([function["bounds"][1]] * dim, dtype=np.float32)
    h = np.array([eps] * dim, dtype=np.float32)

    genotype_to_phenotype = GrayCode().fit(left, right, h)
    str_len = genotype_to_phenotype.parts.sum()

    optimizer = GeneticAlgorithm(
        fitness_function=fitness_,
