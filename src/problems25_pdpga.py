...
from thefittest.optimizers._pdpga import PDPGA
from thefittest.utils.transformations import GrayCode

N_RUNS = int(os.environ.get("PDPGA_N_RUNS", "100"))
MAX_PROBLEMS = int(os.environ.get("PDPGA_MAX_PROBLEMS", "25"))
PROCESSES = int(os.environ.get("PDPGA_PROCESSES", "5"))
EXECUTOR = os.environ.get("PDPGA_EXECUTOR", "thread").lower()
EPS = 0.01
...
def run_optimization_pdpga(function, eps, iters, pop_size):
...
def get_completed_runs(results_file, columns):
    ...
    return {
        (function, int(dimension)): int(count) for (function, dimension), count in grouped.items()
    }
...
def process_problem(problem, runs_to_do):
    ...
    if PROCESSES <= 1:
        for _ in tqdm(range(runs_to_do), desc=problem["function"].__name__, leave=False, ncols=100):
            find_solution, speed_i = run_optimization_pdpga(
                problem,
                EPS,
                problem["iters"],
                problem["pop_size"],
...
if __name__ == "__main__":
    results_file = "pdpga_cec2005.csv"
    columns = [
        "Function",
        "Dimensions",
        "Pop_Size",
